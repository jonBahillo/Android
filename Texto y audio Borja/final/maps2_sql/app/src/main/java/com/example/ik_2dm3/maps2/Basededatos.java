...
public class Basededatos extends SQLiteOpenHelper {
    ...
    public ArrayList<Posiciones> recuperarposicionesuno(int IDentrada){
        ...
        for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
            ...
            Posiciones posicion = new Posiciones(ID,Nombre,Long,Lati,Orden,Imagen,Juego,Pasado,Imagenpasado,Imgenpopup,Juegonombre,Texto);
    ...
    public ArrayList<Rutas> recuperarruta(){
        ArrayList<Rutas> rutas = new ArrayList<Rutas>();

        String myPath = DB_PATH + NOMBRE_BASEDATOS;
        SQLiteDatabase db = SQLiteDatabase.openDatabase(myPath, null, SQLiteDatabase.OPEN_READWRITE);

        Cursor c = db.rawQuery("SELECT * FROM Rutas Order by Posicion", null);
        ...
        for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
            String Idruta = c.getString(c.getColumnIndex("ID"));
            String Posicion = c.getString(c.getColumnIndex("Posicion"));
            String LatiRu =  c.getString(c.getColumnIndex("LatiRu"));
            String LongRu =  c.getString(c.getColumnIndex("LongRu"));

            Rutas ruta = new Rutas(Idruta,Posicion,LatiRu,LongRu);
