...
public class TextoAudiofinal extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_texto_audio_final);
        ...
        audio7 = MediaPlayer.create(TextoAudiofinal.this, R.raw.amaiera1);
        audio7.start();
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                audio7.release();
                audio7 = MediaPlayer.create(TextoAudiofinal.this, R.raw.amaiera2);
                audio7.start();
                //imagensapo.setImageResource(R.drawable.goienkale);
            }
        }, 25200);
        //Log.d("mytag", "Errorrrrrr: " + siguiente7.getTextSize());
        ...
        textomayorfinal = extra1.getBoolean("letra");
        //Log.d("mytag", "Hola " + grande);
        mostrar7.setTextSize(grande);
        if (textomayorfinal){
            mostrar7.setTextSize(18);
        } else{
            mostrar7.setTextSize(13);
        }
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                ejecutar_hilo(mostrar7, pruebatexto7_1, pruebatexto7_2, 680, 2);
            }
        }, 3100);
        handler.postDelayed(new Runnable() {
            @Override
        ...
        siguiente7.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent juego = new Intent(TextoAudiofinal.this, MainActivity.class);
                juego.putExtra("index", index);
                if (audio7.isPlaying()){
                    audio7.stop();
                }
                startActivityForResult(juego, REQ_JUEGO8);
                ...
                Intent intent= null;
                try {
                    intent = new Intent(TextoAudiofinal.this, Class.forName(activity));
                    if (audio7.isPlaying()){
                        audio7.stop();
                    }
                    intent.putExtra("index", index);
                    startActivityForResult(intent, REQ_JUEGO8);
                } catch (ClassNotFoundException e) {
                    e.printStackTrace();
                }
               /* Intent juego = new Intent(TextoAudiofinal.this, TextoAudio1.class);
                if (audio7.isPlaying()){
                    audio7.stop();
                }
